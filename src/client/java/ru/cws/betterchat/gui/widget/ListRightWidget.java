...
public abstract class ListRightWidget extends ChatWidget {
    ...
    public ListRightWidget(ITabListenScreen screen, boolean chatSettings, int width) {
        super(0, 0, width, 20, Text.of("[▶]"), Text.of("Просмотр следующих категорий"));
        this.screen = screen;
    ...
    public static ListRightWidget create(ITabListenScreen screen, boolean chatSettings) {
//        return BetterChatMod.NO_HEAVY_TEXTURES ? new Texted(screen, chatSettings) : new Textured(screen, chatSettings);
    ...
    public void onPress() {
        ...
        if (pos < Math.min(BetterChatMod.CATEGORIES.size(), BetterChatMod.CATEGORIES.size() - (this.chatSettings ? BetterChatMod.CHAT_VIEW_TABS_COUNT : BetterChatMod.SETTINGS_VIEW_TABS_COUNT))) {
