...
public class FlexRender implements IFlexRender {
    ...
    public FlexRender(ISized sized, int gridXSize, int gridYSize) {
        ...
        this.sized = sized;
        this.intensityCache = new float[this.sized.getWidth() * this.sized.getHeight()];
    }
    ...
    private void renderCachedEffect(DrawContext context) {
        final int CELL_WIDTH = this.sized.getWidth() / GRID_X_SIZE;
        final int CELL_HEIGHT = this.sized.getHeight() / GRID_Y_SIZE;

        int index = 0;
        for (int gy = 0; gy < GRID_Y_SIZE; gy++) {
            int y = this.sized.getY() + gy * CELL_HEIGHT;
            for (int gx = 0; gx < GRID_X_SIZE; gx++) {
                int x = this.sized.getX() + gx * CELL_WIDTH;
